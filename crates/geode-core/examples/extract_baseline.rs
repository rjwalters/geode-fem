...
//! 2. Then: `cargo run -p geode-core --example extract_baseline`.
//!
...
/// Workspace root, two parents above `crates/geode-core/`.
...
fn write_toml(rows: &[Row], out_path: &Path) {
    let mut s = String::new();
    s.push_str("# Auto-generated by `cargo run -p geode-core --example extract_baseline`.\n");
    ...
    fs::create_dir_all(out_path.parent().expect("baseline parent")).expect("mkdir");
    fs::write(out_path, s).expect("write baseline.toml");
