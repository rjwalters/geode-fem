%% main.tex — Anvil paper (thread: conformal-antenna-diffopt, version .2)
%% Revised by anvil:paper-revise from .1 against the .1.review critic sibling.
...
faithful discretization is computationally intractable --- a RAM wall by
$R\!\ge\!14$ on a $61$~GB box, $\sim\!10^{15}$ cells at the curve-faithful
resolution). We did not run a converged FDTD-density optimization; the
...
yet in our verified bibliography and are named here for the reader without citing
an unresolved key (see Section~\ref{sec:discussion}).
...
conformal mesh reaches that fidelity at fixed DOF.
...
problem with Meep $1.34.0$ (the vendored \texttt{reference/meep/docker/}
...
\begin{figure}[t]
  ...
  perimeter error). (Placeholder --- rendered by \texttt{paper-figures} from
  \texttt{figures/src/plot\_runtime\_scaling.py}.)}
  \label{fig:runtime}
\end{figure}
...
is computationally intractable ($\sim\!R^3$ cells, $\sim\!R^4$ wall-clock, a RAM
wall on a $61$~GB box, $\sim\!10^{15}$ cells at curve-faithful resolution). The
...
accurate on this curved open radiator --- it cannot practically reach the design
at the resolution the geometry demands.
...
Finally, three references are named in the text but are not yet in our
...
ACS Photonics 2019); (2) the Meep adjoint-solver reference / documentation; and
(3) a canonical antenna shape/topology-optimization reference beyond Ghassemi et
al.\ that would strengthen the parametric-baseline framing.
...
$\sim\!R^4$ wall-clock, hitting a RAM wall well before the curve is resolved.
...
of the GEODE-FEM repository. The headline design result is
