...
agreement. Performance is reported as measured, wins and losses alike: on a
matched same-session head-to-head at the physical target (6 modes,
4.5\,GHz, 133k DOFs), geode-fem's serial direct-factorization eigensolve
on a single core ($28.7$\,s) beats Palace on eight MPI ranks ($44.5$\,s)
in absolute wall clock while consuming ${\sim}12\times$ fewer core-seconds
--- a per-core-efficiency, direct-versus-iterative result, not a
parallelization claim (8-thread geode gives essentially no speedup here).
The direct shift-invert is target-insensitive while Palace's iterative
Krylov+AMS degrades off-target, so the gap \emph{widens} at 20\,GHz
($6.7\times$ serial); the advantage is scale-bounded, inverting at
${\sim}1.16$M DOFs where the direct factorization is OOM-killed and Palace
completes. Meanwhile the GPU driven-solve scaling
...
\begin{itemize}
  ...
  \item \textbf{Honest performance accounting on both axes.} On CPU, a
    matched same-session comparison at the physical target (6 modes,
    4.5\,GHz, 133k DOFs) shows geode-fem's serial direct-factorization
    eigensolve on one core ($28.7$\,s) beating Palace on eight ranks
    ($44.5$\,s) in absolute wall clock at ${\sim}12\times$ fewer
    core-seconds; the direct shift-invert is target-insensitive where
    Palace's iterative solve degrades off-target, and the advantage is
    scale-bounded --- it inverts at ${\sim}1.16$M DOFs where the direct
    factorization is memory-bound (Section~\ref{sec:perf-cpu}). On GPU, a
    same-host four-configuration
...
instance (8 physical cores / 16 vCPUs, ${\sim}61$\,GB usable RAM) in
...
dollars.} The cell is a \emph{matched} head-to-head: on the same box in
the same session, both solvers ran the identical mesh at the identical
eigenproblem, each sampled at 1 and 8 cores/ranks
(\texttt{GEODE\_NUM\_THREADS} $\in \{1, 8\}$ for geode-fem;
\texttt{mpirun -np} $\in \{1, 8\}$ for Palace), at three workloads: the
physical target (6 modes, 4.5\,GHz), an off-target window (12 modes,
20\,GHz), and a uniformly refined large-scale mesh (${\sim}1.16$M interior
DOFs). Each measurement covers the \emph{full pipeline} --- mesh load,
assembly, solve, and output --- timed with \texttt{/usr/bin/time};
timings are the mean over $n = 3$ runs. geode-fem ran at commit
\texttt{3174015}, Palace at commit \texttt{fba6a5b}, both release builds.
The complete matched cell --- per-run wall clocks, peak resident-set
sizes, and instance/software provenance --- is committed as
...
\begin{table}[t]
  \centering
  \caption{Matched CPU-cell wall-clock and memory, full pipeline (mesh load
  + assembly + solve + output), \texttt{m6i.4xlarge}, same box and session.
  geode-fem \texttt{@3174015} uses a serial/threaded faer sparse-LU
  shift-invert Lanczos (direct); Palace \texttt{@fba6a5b} uses distributed
  MPI Krylov + AMS (iterative). Timings are the mean over $n = 3$ runs.
  Palace memory is the per-rank maximum single-process RSS, not an
  aggregate across ranks. At the physical target geode on \emph{one} core
  ($28.7$\,s) beats Palace on \emph{eight} ranks ($44.5$\,s) in absolute
  wall clock, and the gap widens off-target; at ${\sim}1.16$M DOFs the
  direct factorization is OOM-killed (63.9\,GB peak on the ${\sim}61$\,GB
  box) while Palace completes. Values from the committed
  \texttt{benchmarks/transmon\_bench\_cpu/results.toml} (\texttt{[matched.*]}
  tables).}
  \label{tab:cpu}
  \small
  \begin{tabular}{@{}llcc@{}}
    ...
    & geode-fem (1) & OOM-killed & 63.9\,GB \\
    & Palace (8)    & 423.12 & 4.1\,GB/rank \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  core-seconds --- geode on one core consumes $28.7$ core-s against
  Palace's $356.0$ at 8 ranks (${\sim}12\times$). Full-pipeline timings
  from Table~\ref{tab:cpu}.}
  \label{fig:cpu}
\end{figure}

Table~\ref{tab:cpu} supports three readings that must be taken together;
none of them is a ``we parallelized better'' claim.

\paragraph{Per-core efficiency and an absolute single-core win.} At the
physical target, geode-fem's serial direct factorization on a
\emph{single} core ($28.7$\,s) beats Palace on \emph{eight} MPI ranks
($44.5$\,s) in absolute wall clock, and does so while consuming
${\sim}12\times$ fewer core-seconds ($28.7$ versus $356.0$ core-s). This
is a per-core-efficiency and direct-versus-iterative result, explicitly
\emph{not} a parallelization win: geode-fem's own 8-thread run gives
essentially no speedup over its 1-thread run at this target ($29.0$ versus
$28.7$\,s), a limitation tracked as repository issue \#518. The advantage
comes from doing one sparse factorization at high per-core throughput
rather than from spreading work across cores.

\paragraph{Target robustness.} The direct shift-invert is
\emph{target-insensitive} --- the cost of a single factorization at the
shift is nearly independent of where the target sits --- so moving from 6
modes at 4.5\,GHz to 12 modes at the far-off-target 20\,GHz barely changes
geode-fem's serial time ($28.7 \to 36.8$\,s). Palace's iterative
Krylov+AMS \emph{degrades} off-target, where a poorly placed shift slows
convergence ($130.9 \to 248.0$\,s at np1; $44.5 \to 64.7$\,s at np8).
Consequently the gap \emph{widens} off-target --- $6.7\times$ serial
...
Palace wins at scale on memory, completing where the direct path OOMs.
Correctness is measured separately and not conflated with these timings
--- eigenvalue agreement across all physical modes ($\leq 0.032\%$) is
cross-validated on the identical mesh in
\path{benchmarks/transmon_eigen/results.toml}
(Section~\ref{sec:agreement}), not in this performance cell. Palace at 16
ranks on the hyperthreaded vCPUs was refused by MPI process binding; we
exclude it rather than report a degraded configuration as if it were a
measurement.
...
The measured position is: per-core CPU competitiveness demonstrated (a
single geode-fem core at $28.7$\,s beats the distributed reference on
eight ranks at $44.5$\,s at the 133k-DOF physical target, at
${\sim}12\times$ fewer core-seconds, with the gap widening off-target and
inverting only at ${\sim}1.16$M DOFs where the direct factorization is
memory-bound), GPU
...
rather than eliminated. (iv) The per-core efficiency and single-core-beats-eight-ranks result is
specific to the small-to-medium regime where a serial direct
factorization is memory-feasible; it is explicitly scale-bounded and
inverts at ${\sim}1.16$M DOFs, where the direct path is OOM-killed and the
distributed iterative reference completes. (v) The GPU evidence covers driven-solve correctness and an
...
physics; it beats the reference on per-core efficiency (one core at $28.7$\,s
versus eight ranks at $44.5$\,s at the 133k-DOF target, ${\sim}12\times$
fewer core-seconds, a target-robust direct-versus-iterative advantage that
is scale-bounded at ${\sim}1.16$M DOFs); and it gains a portable CUDA execution path from the same
