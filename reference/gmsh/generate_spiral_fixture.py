#!/usr/bin/env python3
"""Generate + validate the spiral-inductor FEM mesh fixture (issue #210).

Offline generation wrapper around ``spiral_inductor.geo``: gmsh is NOT a
...
Pipeline:
  1. read spiral + layer-stack parameters from a PDK-style YAML file
     (flat two-level subset parsed with the stdlib — no PyYAML needed),
  2. invoke the ``gmsh`` CLI on ``spiral_inductor.geo`` with
     ``-setnumber`` overrides,
...
GEO_SCRIPT = SCRIPT_DIR / "spiral_inductor.geo"

# Quality gates.
MAX_EDGES = 100_000  # direct sparse LU affordability target (issue #210)
...
EXPECTED_GROUPS = [
    ...
    (3, 4, "air_buffer"),
    (2, 11, "port"),
    (2, 12, "conductor_surface"),
    (2, 13, "outer_boundary"),
]

# YAML keys (under `spiral:` / `stack:` / `mesh:`) forwarded to the .geo
# as -setnumber parameters.
GEO_PARAMS = {
    "spiral": ["n_turns", "w", "s", "d_in", "g", "feed_len", "stub_len"],
    "stack": ["h_sub", "h_ox", "z1_bot", "t1", "z2_bot", "t2", "h_air", "h_buf", "margin"],
    "mesh": ["lc_cond", "lc_port", "lc_far", "dist_far"],
}
...
def parse_simple_yaml(path: Path) -> dict:
    ...
    general YAML parser — it covers exactly the PDK-style schema of
    ``spiral_3p5_generic.yaml`` so the wrapper has no dependencies
...
def parse_msh(path: Path):
    ...
    nodes: {tag: (x, y, z)};  tets/tris: list of (phys_tag, [node tags]);
    group_names: {(dim, tag): name}.
    """
    text = path.read_text()
...
def min_dihedral_deg(v) -> float:
    ...
    The dihedral along edge (i, j) is the angle between the two faces
    sharing that edge, computed from their (outward-agnostic) normals.
    """
    ...
    for a, b, c in faces:
        ...
        normals.append(n)
    worst = 180.0
    # Faces k and l share the edge opposite to vertices k, l.
    for k in range(4):
...
def main(argv: list) -> int:
    ...
    prov = [
        ...
        f"script:         reference/gmsh/spiral_inductor.geo",
        f"wrapper:        reference/gmsh/generate_spiral_fixture.py",
        f"parameters:     {yaml_path.name}",
        "",
        "parameter values:",
    ]
    for section in ("spiral", "stack", "mesh", "materials"):
