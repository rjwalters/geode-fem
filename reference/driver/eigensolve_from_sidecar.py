"""Backend-agnostic eigensolve driver for cube-cavity sidecars (Epic #88 / issue #127).

Consolidates ``eigensolve_from_tfjava.py`` and ``eigensolve_from_onnx.py`` into a
single script parameterised by ``--backend``. The two existing scripts are thin
wrappers that inject ``--backend tfjava`` / ``--backend onnx`` and delegate here,
so all CI invocations and external callers continue to work without change.
...
The output JSON is in fixture-schema v1 so the harness can compare it to the
JAX / NumPy baselines without language-specific code paths (see
...
def main():
    parser = argparse.ArgumentParser(
        description="Backend-agnostic SciPy eigensolve over a reduced_kM.json sidecar."
    )
    parser.add_argument(
        "sidecar",
