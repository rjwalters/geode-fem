...
//!
//! This is a standalone example crate (`examples/mie_driven_scattering/`)
//! built on the `geode-app` harness, migrated from the old
//! `crates/geode-core/examples/mie_driven_scattering.rs` (Epic #398
//! Phase 3a). The physics, report output, and `driven_results*.toml`
//! artifacts are preserved exactly; only the entry point (hand-rolled
//! positional-`fine` argv scan → `clap` derive + `geode_app::App`)
//! changed.
//!
//! `--release` is recommended: the dense linear algebra in the
//! scattered-field solve is far slower under `debug-assertions`.
//!
//! Run with:
//!
//! ```sh
//! cargo run -p mie_driven_scattering --release
//! ```
//!
//! Passing `--fine` selects the finer bundled fixture
...
/// the example-local fixture selector (`--fine`) the original hand-rolled
/// positional `fine` argument recognised.
#[derive(Parser)]
#[command(
    about = "Driven Mie scattering benchmark: Q_ext / Q_sca vs. ka against the analytic Mie series (issue #195)."
)]
struct Args {
    /// Run on the finer bundled fixture (~5.9k nodes, issue #215) and
    /// write `driven_results_fine.toml` instead of `driven_results.toml`.
    #[arg(long)]
    fine: bool,
...
impl App for Args {
    fn run(self) -> Result<(), Box<dyn std::error::Error>> {
        let choice = if self.fine {
            FixtureChoice::Fine
        } else {
            FixtureChoice::Coarse
        };
        let fixture = match choice {
            FixtureChoice::Coarse => read_sphere_fixture()?,
            FixtureChoice::Fine => read_sphere_fine_fixture()?,
        };
        eprintln!(
